...
public class FileUploadController {
	...
	public Response uploadFile(@Context HttpServletRequest request) {
		...
		try {
			...
			for (Part part : request.getParts()) {
				log.info("Part name: {}, size: {}, content type: {}", part.getName(), part.getSize(),
						part.getContentType());
			}
			...
			// 6. 重新將字串轉換為輸入流進行解析
			List<ExchangeTransaction> transactions = csvParsingService
					.parseCSV(IOUtils.toInputStream(fileString, StandardCharsets.UTF_8));
	...
	private Response createErrorResponse(String message) {
		return Response.status(Response.Status.BAD_REQUEST).entity(new UploadResult(message)).build();
	}
}